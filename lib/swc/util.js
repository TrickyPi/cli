...
exports.getSwcrcInfo = getSwcrcInfo;
...
function outputFile(output, filename, sourceMaps) {
  ...
  if (output.map && sourceMaps !== "inline") {
    ...
    code +=
      "\n//# sourceMappingURL=" +
      (0, _slash).default((0, _path).relative(fileDirName, mapLoc));
...
function getSwcrcInfo() {
  const fileLocation = (0, _path).resolve(process.cwd(), ".swcrc");
  if (!(0, _fs).statSync(fileLocation)) return false;
  try {
    const info = (0, _fs).readFileSync(fileLocation, "utf-8");
