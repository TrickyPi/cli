...
async function handleCompile(filename, outDir, sync, swcOptions) {
  ...
  const sourceFileName = (0, _slash).default(
    (0, _path).relative((0, _path).dirname(dest), filename)
  );
  const options = {
    ...swcOptions,
    sourceFileName,
...
async function initialCompilation(cliOptions, swcOptions) {
  ...
  const sourceFiles = await (0, _sources).globSources(
    filenames,
    includeDotfiles
  );
  ...
  if (!quiet && compiled1 + copied1) {
    let message = "";
    if (compiled1) {
      message += `Successfully compiled: ${compiled1} ${
        compiled1 > 1 ? "files" : "file"
      }`;
    }
...
async function watchCompilation(cliOptions, swcOptions) {
  ...
  watcher.on("unlink", async filename => {
    try {
      if ((0, _sources).isCompilableFile(filename, extensions, swcOptions)) {
