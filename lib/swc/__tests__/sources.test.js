...
describe("globSources", () => {
  ...
  it("exclude dotfiles sources when includeDotfiles=false", async () => {
    const files = await (0, _sources).globSources([".dotfile"], false);
  ...
  it("include dotfiles sources when includeDotfiles=true", async () => {
    ...
    const files = await (0, _sources).globSources([".dotfile"], true);
  ...
  it("include multiple file sources", async () => {
    ...
    const files = await (0, _sources).globSources([".dotfile", "file"], true);
  ...
  it("includes all files from directory", async () => {
    ...
    _fastGlob.default.mockResolvedValue(["fileDir1", "fileDir2"]);
    const files = await (0, _sources).globSources(["file", "directory"], true);
...
describe("slitCompilableAndCopyable", () => {
  ...
  it("separate compilable and copyable when copyFiles=true", () => {
    ...
    const [compilable, copyable] = (0, _sources).slitCompilableAndCopyable(
      files,
      extensions,
      {},
      true
