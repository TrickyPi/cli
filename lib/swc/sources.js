...
async function globSources(sources, includeDotfiles = false) {
  ...
  const files = await Promise.all(
    sources
      .filter(
        source =>
          includeDotfiles || !(0, _path).basename(source).startsWith(".")
      )
      .map(source => {
        return new Promise(resolve => {
          (0, _fs).stat(source, (err, stat) => {
            ...
            if (!stat.isDirectory()) {
              ...
              (0, _fastGlob)
                .default(
                  (0, _slash).default((0, _path).join(source, "**")),
                  globConfig
                )
...
function isExclude(filename, { exclude }) {
  if (!exclude) {
    return false;
  }
...
function isCompilableFile(filename, allowedExtension, swcOptions) {
  return (
    isCompilableExtension(filename, allowedExtension) &&
    !isExclude(filename, swcOptions)
  );
}
function slitCompilableAndCopyable(
  files,
  allowedExtension,
  swcOptions,
  copyFiles
...
async function watchSources(sources, includeDotfiles = false) {
  ...
  return chokidar.watch(sources, {
    ignored: includeDotfiles
      ? undefined
      : filename => (0, _path).basename(filename).startsWith("."),
    ignoreInitial: true,
    awaitWriteFinish: {
